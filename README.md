...
把一句模糊需求，整理成可交给 AI agent 独立执行的任务说明；也可以把大任务整理成带边界、验收、授权等级和子智能体派工规则的托管执行计划。

这个仓库收录了一个 Codex skill：`agent-task-spec`。

它解决的不是“怎么把 prompt 写长”，而是“怎么把任务交代清楚，让 agent 能真的把事做完”。一个好的 Agent Task Spec 会把目标、背景、可操作范围、暂停条件、假完成提醒、验证证据和交付格式都写明白，减少 agent 猜错、乱改、只交计划不执行、或者说“完成了”但没有证据的情况。

最短用法：

```text
用 $agent-task-spec 把这个任务整理成可执行 spec：
...
```text
用 $agent-task-spec 的托管执行模式处理这个任务，然后按 spec 执行：
...
- 你想让 agent 接管更多工作：主动读文件、查资料、运行命令、改文件、验证结果
- 你想让主 Agent 合理派子智能体，而不是一窝蜂并行乱改
...
默认会把需求整理成类似这样的结构。小任务会压缩成精简版，大任务才展开完整结构。
...
## Execution Contract
agent 的自主执行等级、是否可以派子智能体、失败后怎么重试。
...
4. **Managed Execution mode**

   你希望 agent 不只是写建议，而是接管执行：先写清目标、边界和验收，再主动读取资料、运行命令、修改文件、验证结果；任务能独立拆分时再派子智能体，主 Agent 负责最终判断、整合和汇报。

## 托管执行等级

当你使用 `Managed Execution mode` 时，可以指定 agent 的自主程度：

| 等级 | 含义 | 适合场景 |
| --- | --- | --- |
| `L1 Spec only` | 只整理任务说明，不执行 | 你只是想拿一份 prompt/spec |
| `L2 Read and research` | 可读文件、查资料、出方案，不改文件 | 调研、审查、风险评估 |
| `L3 Local execution` | 可运行本地命令、修改明确范围内的文件并验证 | 常规代码、文档、内容、自动化任务 |
| `L4 Delegated execution` | 可在 L3 基础上派子智能体处理独立模块 | 大型任务、多个独立研究/开发模块 |
| `L5 Pause required` | 触发高风险动作时必须暂停确认 | 账号、付费、生产、删除、发布、隐私数据等 |

如果你不指定等级，skill 会根据措辞选择保守但可推进的默认值。一般来说，“直接帮我操作”“接管”“按 spec 执行”会被理解为 `L3`；只有任务天然能拆成独立模块时，才建议升到 `L4`。

## 子智能体派工规则

子智能体不是越多越好。这个 skill 会倾向于这样使用：

- 主 Agent 保留关键路径和最终判断。
- 只有任务能拆成互不依赖的模块时才派子智能体。
- 每个子智能体必须有清楚的目标、范围、可改文件和不可触碰范围。
- 多个子智能体不能负责同一批文件，避免互相覆盖。
- 子智能体输出不能直接当结论，主 Agent 必须复核、整合、验证。
...
如果你希望 agent 接管更多动作，用 `Managed Execution mode`：

```text
用 $agent-task-spec 的托管执行模式处理这个任务。先写清目标、边界、可直接操作范围、需要暂停确认的事项、子智能体派工规则和验收方式，然后按这份 spec 执行。
...
- `Execution Contract`：agent 可以自主操作什么，什么时候派子智能体，什么情况必须暂停确认。
...
```text
用 $agent-task-spec 的托管执行模式处理下面这个任务，然后按 spec 执行。默认使用 L3；如果任务能拆成互不影响的独立模块，再升级到 L4 派子智能体。

要求：
- 先明确目标、边界、验收标准和暂停条件。
- 能直接操作的就直接操作，不只给建议。
- 可以读取文件、搜索公开资料、运行本地命令、修改任务相关文件、启动本地服务并做验证。
- 只有账号、付费、生产数据、删除、隐私、不可逆操作、公开发布或大范围重写时才暂停问我。
- 能并行拆分时再派子智能体；每个子智能体必须有独立范围，避免多人改同一文件。
- 主 Agent 负责最终验收、合并判断和汇报证据。
...
```text
用 $agent-task-spec 的托管执行模式，按 L3 执行：

[写你的任务]
```

```text
用 $agent-task-spec 的托管执行模式，按 L4 执行。只有互不依赖的模块才派子智能体，每个子智能体必须有独立文件范围，主 Agent 最后复核和验证：
...
```text
用 $agent-task-spec 的托管执行模式：帮我把这个资料库学习下来，形成可执行计划；能并行研究的部分再派子智能体，主 Agent 最后整合、验证和汇报。
```

### 审查一段容易跑偏的任务

```text
用 $agent-task-spec 审查下面这段任务。请指出它为什么容易让 agent 假完成、乱改或过度追问，并给我一版更适合托管执行的改写：
...
- 不滥用子智能体：只有任务能拆成独立模块时才并行派工，主 Agent 始终负责验收。
- 不把托管执行等同于无限授权：账号、付费、生产、删除、发布、隐私数据和不可逆操作必须暂停确认。
